...
{
  if(len % PGSIZE != 0)
    panic("munmap: len not page aligned");

  struct proc *p = myproc();
  struct vm_area *vmas = p->vma;
  struct vm_area *vma = 0;
  for(int i = 0; i < NVMA; i++){
    if(vmas[i].start <= addr && addr < vmas[i].end){
      vma = &vmas[i];
      break;
    }
  }
  ...
  if(vma->flags == MAP_SHARED){
    ...
    if(ip == 0)
      panic("munmap: no file");
    begin_op();
    ilock(ip);
    writei(ip, 1, addr, vma->offset + addr - vma->start, len);
    iunlock(ip);
    end_op();
  }
  uint64 start = PGROUNDUP(addr);
  uint64 left = len - (start - addr);
  uint64 npages = left / PGSIZE;
  if(start + left >= vma->end && left % PGSIZE != 0)
    npages++;
...
int pgfaulthandler(void)
{
  ...
  struct inode *ip = vma->file->ip;
  if (ip == 0)
    panic("Page fault: no file");
  ilock(ip);
  readi(ip, 0, pa, vma->offset + va - vma->start, PGSIZE);
  iunlock(ip);
  int prem = PTE_U;
  ...
  if (vma->prot & PROT_READ)
    prem |= PTE_R;
  mappages(p->pagetable, va, PGSIZE, pa, prem);
  return 0;
}
